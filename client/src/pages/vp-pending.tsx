...
export default function VPPendingPage() {
  ...
  const { data: rejectedByVPRaw = [], isLoading: loadingRejected } = useQuery<ReleaseOrderEntry[]>({
    queryKey: ["/api/release-orders", { rejectedById: user?.id?.toString() }],
    enabled: !!user?.id,
  });

  // Filter rejected orders: only show those that are still rejected (not pending_vp_review again)
  // If a rejected order comes back for approval (status = pending_vp_review), don't show it in reject section
  const rejectedByVP = rejectedByVPRaw.filter((entry) => {
    const status = entry.releaseOrder?.status;
    // Only show if status is NOT pending_vp_review (meaning it hasn't come back for approval)
    return status !== "pending_vp_review";
  });
      ...
      <Tabs defaultValue="pending" className="w-full">
        ...
        <TabsContent value="reject" className="space-y-4">
          {loadingRejected ? (
            ...
            renderReleaseOrders(rejectedByVP, setSelectedEntry, false)
          )}
        </TabsContent>
      </Tabs>
